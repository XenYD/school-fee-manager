...
import {
  ...
  Search, Filter, BadgeDollarSign, ListFilter, CheckCheck
} from 'lucide-react'
    ...
    try {
      ...
      if (student.fee_record) {
        const { error } = await supabase
          .from('fee_records')
          .update({ paid: !isPaid, paid_date: !isPaid ? new Date().toISOString() : null })
      ...
      toast.success(isPaid ? 'Marked as unpaid' : 'Fee collected! ✓')
      loadData()
    } catch (err: unknown) {
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              {paidStudents.length}/{students.length} paid
            </p>
          </div>
          <button onClick={() => changeMonth(1)} className="p-2 rounded-lg hover:bg-gray-100 text-gray-600 transition-colors">
        ...
        <div className="flex gap-2">
          ...
          <div className="flex bg-gray-100 rounded-lg p-0.5">
            {(['all', 'paid', 'unpaid'] as FilterType[]).map((f) => (
              ...
              >
                {f === 'all'
                  ? `All (${classFilteredStudents.length})`
                  : f === 'paid'
      ...
      {students.length === 0 ? (
        ...
        <div className="card text-center py-8">
          ...
          <p className="text-gray-500 text-sm">
            {selectedClass && classFilteredStudents.length === 0
        ...
        <div className="space-y-2.5">
          {displayStudents.map((student) => {
            ...
            const paidDate = student.fee_record?.paid_date
              ? new Date(student.fee_record.paid_date).toLocaleDateString()
              : null
            ...
            return (
              <div
                ...
                className={`bg-white rounded-xl border transition-all ${
                  paid ? 'border-green-200' : 'border-gray-200'
                } p-4 sm:p-4`}
              >
                <div className="flex items-center gap-3">
                  {/* Avatar */}
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
                    paid ? 'bg-green-100' : 'bg-gray-100'
                  }`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      {paidDate && (
                        <span className="text-xs text-green-600">• Paid on {paidDate}</span>
                      )}
                    </div>
                  </div>
