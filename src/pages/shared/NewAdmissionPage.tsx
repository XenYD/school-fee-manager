...
export default function NewAdmissionPage() {
  ...
  async function loadStudents() {
    const { data } = await supabase
      .from('students').select('id, name, class, sibling_ids')
  ...
  function toggleSibling(id: string) {
    setField('sibling_ids', form.sibling_ids.includes(id)
      ? form.sibling_ids.filter((s) => s !== id)
      : [...form.sibling_ids, id])
  }
  ...
  function validateStep1() {
    ...
    if (!form.class) { toast.error('Class is required'); return false }
        ...
        {steps.map((s, idx) => (
          <div key={s.n} className="flex items-center flex-1">
            <div className="flex flex-col items-center flex-shrink-0">
              ...
              <p className="text-xs font-medium mt-1 text-center hidden sm:block"
    ...
    return (
      <div className="card space-y-4">
        ...
        <div>
          ...
          <input className={inputCls} placeholder="Student full name" autoFocus
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input type="number" className={inputCls} placeholder="e.g. 3500"
              value={form.fee_amount} onChange={(e) => setField('fee_amount', e.target.value)} />
          </div>
          <div>
            ...
            <input type="number" className={inputCls} placeholder="e.g. 500"
              value={form.exam_fee_amount} onChange={(e) => setField('exam_fee_amount', e.target.value)} />
          </div>
        </div>
      </div>
    )
      ...
      <div className="card space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input className={inputCls} placeholder="Full name" autoFocus
              value={form.parent_name} onChange={(e) => setField('parent_name', e.target.value)} />
          </div>
          <div>
            <label className={labelCls} style={{ color: 'var(--c-text-2)' }}>CNIC <span className="text-red-400">*</span></label>
            <input className={inputCls} placeholder="42101-1234567-1"
              value={form.parent_cnic} onChange={(e) => setField('parent_cnic', e.target.value)} />
          </div>
          <div>
            ...
            <input className={inputCls} placeholder="03XX-XXXXXXX"
              value={form.parent_phone} onChange={(e) => setField('parent_phone', e.target.value)} />
          </div>
          <div>
            <label className={labelCls} style={{ color: 'var(--c-text-2)' }}>WhatsApp Number</label>
            <input className={inputCls} placeholder="Same or different number"
              value={form.parent_whatsapp} onChange={(e) => setField('parent_whatsapp', e.target.value)} />
          </div>
          <div className="sm:col-span-2">
            <label className={labelCls} style={{ color: 'var(--c-text-2)' }}>Home Address <span className="text-red-400">*</span></label>
            <textarea className={inputCls} rows={3} placeholder="Full home address"
        ...
        <div className="card space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select className={inputCls} value={form.blood_group} onChange={(e) => setField('blood_group', e.target.value)}>
            ...
            <div>
              ...
              <input className={inputCls} placeholder="e.g. Islam"
            ...
            <div>
              ...
              <input className={inputCls} placeholder="03XX-XXXXXXX"
                value={form.emergency_contact_phone} onChange={(e) => setField('emergency_contact_phone', e.target.value)} />
            </div>
            <div>
              <label className={labelCls} style={{ color: 'var(--c-text-2)' }}>Previous School</label>
              <input className={inputCls} placeholder="Name of previous school"
                value={form.previous_school} onChange={(e) => setField('previous_school', e.target.value)} />
            </div>
            <div>
              ...
              <textarea className={inputCls} rows={2} placeholder="Leave blank if none"
                value={form.special_needs} onChange={(e) => setField('special_needs', e.target.value)} />
            </div>
          </div>
        </div>
        ...
        {existingStudents.length > 0 && (
          <div className="card space-y-3">
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <input className={inputCls + ' pl-8'} placeholder="Search by name…"
                  value={sibSearch} onChange={(e) => setSibSearch(e.target.value)} />
              </div>
              <select className={inputCls + ' w-32'} value={sibClass} onChange={(e) => setSibClass(e.target.value)}>
            ...
            {filteredSiblings.length === 0 ? (
              <p className="text-xs text-center py-2" style={{ color: 'var(--c-text-4)' }}>No students found</p>
            ) : (
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Step {step} of 3 — {steps[step - 1].label}</p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between pt-2">
        ...
        {step < 3 ? (
          <button type="button" onClick={nextStep} className="btn-primary text-sm px-8 flex items-center gap-2">
            Next <ArrowRight size={15} />
          </button>
        ) : (
          <button type="button" onClick={handleSubmit} disabled={saving} className="btn-primary text-sm px-8 flex items-center gap-2">
            {saving
